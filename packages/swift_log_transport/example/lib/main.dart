...
class _MyAppState extends State<MyApp> {
  ...
  final _swiftLogTransport = SwiftLogTransport();
  ...
  Future<void> logSample() async {
    try {
      final event = LogEvent(
        level: LogLevel.info,
        message: 'Hello from SwiftLogTransport!',
        context: 'ExampleApp',
        timestamp: DateTime.now(),
      );
      await _swiftLogTransport.log(event);
      if (!mounted) return;
      setState(() {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: Scaffold(
        appBar: AppBar(title: const Text('SwiftLogTransport Example')),
        body: Center(
          child: Column(
