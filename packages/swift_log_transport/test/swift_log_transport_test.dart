...
void main() {
  ...
  setUp(() {
    fake = _FakePlatform();
    SwiftLogTransportPlatform.instance = fake;
  });

  group('SwiftLogTransport', () {
    // --- constructor ---

    test('can be instantiated with default config', () {
      expect(() => SwiftLogTransport(), returnsNormally);
    });

    test('label defaults to null', () {
      expect(SwiftLogTransport().label, isNull);
    });

    test('label read from config', () {
      expect(
        SwiftLogTransport(config: {'label': 'com.example.app'}).label,
        'com.example.app',
      );
    });

    test('metadata defaults to null', () {
      expect(SwiftLogTransport().metadata, isNull);
    });

    test('metadata read from config', () {
      final meta = {'env': 'prod', 'version': '1.0'};
      expect(
        SwiftLogTransport(config: {'metadata': meta}).metadata,
        meta,
      );
    });

    test('format defaults to {message}', () {
      expect(SwiftLogTransport().format, '{message}');
    });

    test('format read from config', () {
      expect(
        SwiftLogTransport(config: {'format': '[{level}] {message}'}).format,
        '[{level}] {message}',
      );
    });
    ...
    test('does not forward event below threshold', () async {
      final t = SwiftLogTransport(level: LogLevel.error);
      await t.log(LogEvent(level: LogLevel.debug, message: 'nope'));
      expect(fake.calls, isEmpty);
    });

    test('forwards event at threshold level', () async {
      final t = SwiftLogTransport(level: LogLevel.warn);
      await t.log(LogEvent(level: LogLevel.warn, message: 'warn'));
      expect(fake.calls, hasLength(1));
    });

    test('forwards event above threshold level', () async {
      final t = SwiftLogTransport(level: LogLevel.info);
      await t.log(LogEvent(level: LogLevel.fatal, message: 'fatal'));
      expect(fake.calls, hasLength(1));
    });

    // --- payload fields ---

    test('level field matches event level name for all levels', () async {
      final t = SwiftLogTransport();
    ...
    test('label field matches configured label', () async {
      final t = SwiftLogTransport(config: {'label': 'com.example'});
      await t.emitLog(LogEvent(level: LogLevel.info, message: 'msg'));
      expect(fake.calls.first['label'], 'com.example');
    });

    test('label field is null when not configured', () async {
      final t = SwiftLogTransport();
      await t.emitLog(LogEvent(level: LogLevel.info, message: 'msg'));
      expect(fake.calls.first['label'], isNull);
    });

    test('metadata field matches configured metadata', () async {
      final meta = {'env': 'staging'};
      final t = SwiftLogTransport(config: {'metadata': meta});
      await t.emitLog(LogEvent(level: LogLevel.info, message: 'msg'));
      expect(fake.calls.first['metadata'], meta);
    });

    test('metadata field is null when not configured', () async {
      final t = SwiftLogTransport();
      await t.emitLog(LogEvent(level: LogLevel.info, message: 'msg'));
      expect(fake.calls.first['metadata'], isNull);
    });
    ...
    test('error field contains exception string when present', () async {
      ...
      await t.emitLog(
        LogEvent(level: LogLevel.error, message: 'request failed', error: err),
      );
    ...
    test('stackTrace field contains string when present', () async {
      final t = SwiftLogTransport();
      final trace = StackTrace.fromString('#0 main (main.dart:1:1)');
      await t.emitLog(
    ...
    test('format replaces {level}', () async {
      final t = SwiftLogTransport(config: {'format': '[{level}] {message}'});
    ...
    test('format replaces {context}', () async {
      final t = SwiftLogTransport(config: {'format': '{context}: {message}'});
      await t.emitLog(
        LogEvent(level: LogLevel.info, message: 'ping', context: 'svc'),
      );
      expect(fake.calls.first['message'], 'svc: ping');
    });
    ...
    test('format replaces {timestamp}', () async {
      final ts = DateTime.parse('2024-06-01T00:00:00.000Z');
      final t = SwiftLogTransport(config: {'format': '{timestamp}'});
      await t.emitLog(
        LogEvent(level: LogLevel.info, message: 'msg', timestamp: ts),
      );
    ...
    test('format replaces {error} when present', () async {
      final t = SwiftLogTransport(config: {'format': '{message} ({error})'});
    ...
    test('format replaces {stackTrace} when present', () async {
      final t = SwiftLogTransport(config: {'format': '{message}|{stackTrace}'});
